...
class CustomGPM(nn.Module):
    ...
    def forward(self, observation, action, mode): #mode = {"critic1", "critic2", "actor"}
        ...
        if mode == "actor":
            ...
            output = self.critic_final1(output)  # shape [N, 1]
            ...
            output = self.critic_final2(output)  # shape [N, 1]
            ...
            output = self.value_final(output)  # shape [N, 1]
    ...
    def track_critic_parameters(self):
        pf_params = list(self.short_term.parameters()) + list(self.mid_term.parameters()) + list(
            self.long_term.parameters()) + list(self.gcn.parameters())
        critic1_params = list(self.critic_conv1.parameters()) + list(self.critic_final1.parameters())
        critic2_params = list(self.critic_conv2.parameters()) + list(self.critic_final2.parameters())
        return pf_params + critic1_params + critic2_params

    def track_critic1_parameters(self):
        pf_params = list(self.short_term.parameters()) + list(self.mid_term.parameters()) + list(
            self.long_term.parameters()) + list(self.gcn.parameters())
        critic1_params = list(self.critic_conv1.parameters()) + list(self.critic_final1.parameters())
        return pf_params + critic1_params
